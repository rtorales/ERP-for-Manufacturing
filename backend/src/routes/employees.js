...
router.post(
  '/bulk-import',
  wrapAsync(async (req, res) => {
    await EmployeesService.bulkImport(req, res, true, req.headers.referer);
    const payload = true;
    res.status(200).send(payload);
  }),
);
...
router.get(
  ...
  wrapAsync(async (req, res) => {
    ...
    const payload = await EmployeesDBApi.findAll(req.query);
    if (filetype && filetype === 'csv') {
      const fields = ['id', 'employee_name', 'role', 'shift', 'pay_rate'];
      const opts = { fields };
      try {
