...
import { loadColumns } from './configureQuality_checksCols';
import _ from 'lodash';
import dataFormatter from '../../helpers/dataFormatter';

const perPage = 5;

const TableSampleQuality_checks = ({
  filterItems,
  setFilterItems,
  filters,
}) => {
  ...
  const {
    ...
    loading,
    count,
    notify: quality_checksNotify,
    refetch,
  } = useAppSelector((state) => state.quality_checks);
  ...
  useEffect(() => {
    if (quality_checksNotify.showNotification) {
      notify(
        quality_checksNotify.typeNotification,
        quality_checksNotify.textNotification,
      );
    }
  }, [quality_checksNotify.showNotification]);
  ...
  const handleEditAction = (id: string) => {
    router.push(`/quality_checks/${id}`);
  };

  const handleViewAction = (id: string) => {
    router.push(`/quality_checks/quality_checks-view/?id=${id}`);
  };
  ...
  console.log('quality_checks: ', quality_checks);
...
export default TableSampleQuality_checks;
