...
import { loadColumns } from './configureInventoriesCols';
import _ from 'lodash';
import dataFormatter from '../../helpers/dataFormatter';

const perPage = 5;

const TableSampleInventories = ({ filterItems, setFilterItems, filters }) => {
  ...
  useEffect(() => {
    if (inventoriesNotify.showNotification) {
      notify(
        inventoriesNotify.typeNotification,
        inventoriesNotify.textNotification,
      );
    }
  }, [inventoriesNotify.showNotification]);
  ...
  const handleViewAction = (id: string) => {
    router.push(`/inventories/inventories-view/?id=${id}`);
  };
...
export default TableSampleInventories;
